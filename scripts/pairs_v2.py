#!/usr/bin/env python3
"""Generate and save questions in 2-chapter chunks."""
...
def run_pass(exam_id, ch_map, c1, c2, name, t1, t2):
    print(f'\n=== {name} ===')
    prompt = f"Generate 20 MCQ each for {t1} (chapter={c1}) and {t2} (chapter={c2}). Each: question, options[\"A)t\",\"B)t\",\"C)t\",\"D)t\"], correctAnswer(A-D), explanation, type=MCQ, difficulty, chapter=INTEGER. Return JSON questions array."
    ...
    for attempt in range(3):
        try:
            ...
            for ch in [c1, c2]:
                ...
                if not qs:
                    print(f'  ⚠️ No questions for Ch{ch}')
                    continue
                ch_id = ch_map.get(ch)
                ...
                for q in qs:
                    ...
                    nq = {'question':q.get('question',''),'options':opts,'correctAnswer':str(q.get('correctAnswer','A')).strip().upper()[-1:],'explanation':q.get('explanation','') or 'Reference: TC standards.','type':'MCQ','difficulty':q.get('difficulty','MEDIUM')}
...
TOTAL = 0
TOTAL += run_pass(AF_EXAM, CH_AF, 7, 8, "M-AIRFRAME Ch7-8", "Ch7=Landing Gear & Brakes", "Ch8=Warning Systems & Ice/Rain Protection")
TOTAL += run_pass(AF_EXAM, CH_AF, 9, 10, "M-AIRFRAME Ch9-10", "Ch9=Cabin Pressurization & Oxygen", "Ch10=Aircraft Fuel Systems")
TOTAL += run_pass(AF_EXAM, CH_AF, 11, 12, "M-AIRFRAME Ch11-12", "Ch11=Flight Controls", "Ch12=Fire Protection Systems")
TOTAL += run_pass(PP_EXAM, CH_PP, 7, 8, "M-POWERPLANT Ch7-8", "Ch7=Turbine Engine Lubrication & Sealing", "Ch8=Turbine Fuel Controls & FADEC")
TOTAL += run_pass(PP_EXAM, CH_PP, 9, 10, "M-POWERPLANT Ch9-10", "Ch9=Turbine Ignition & Starting", "Ch10=Engine Indicating & Instrumentation")
TOTAL += run_pass(PP_EXAM, CH_PP, 11, 12, "M-POWERPLANT Ch11-12", "Ch11=Propeller Systems", "Ch12=Engine Installation, Fire Protection & Troubleshooting")

print(f'\n{"="*50}')
print(f'🔥 TOTAL: {TOTAL} new questions')
