          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none pl-4 pr-10 py-2.5 bg-white border border-border rounded-btn text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-blue/50 focus:border-blue transition-colors cursor-pointer"
                >
                  <option value="" className="bg-white text-text-primary">
                    {loadingExams ? 'Chargement des examens...' : 'Sélectionnez un examen'}
                  </option>
                  {exams.map((exam) => (
                    <option key={exam.id} value={exam.id} className="bg-white text-text-primary">
                      {exam.code} — {exam.name}
                    </option>
                  ))}
                </select>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none pl-4 pr-10 py-2.5 bg-white border border-border rounded-btn text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-blue/50 focus:border-blue transition-colors cursor-pointer disabled:opacity-50"
                >
                  <option value="" className="bg-white text-text-primary">
                    {!examId ? "Sélectionnez d'abord un examen" : chapters.length === 0 ? 'Aucun chapitre trouvé' : 'Sélectionnez un chapitre'}
                  </option>
                  {chapters.map((ch) => (
                    <option key={ch.id} value={ch.id} className="bg-white text-text-primary">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none pl-4 pr-10 py-2.5 bg-white border border-border rounded-btn text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-blue/50 focus:border-blue transition-colors cursor-pointer"
                  >
                    <option value="MCQ" className="bg-white text-text-primary">MCQ</option>
                    <option value="TRUEFALSE" className="bg-white text-text-primary">Vrai / Faux</option>
                    <option value="WRITTEN" className="bg-white text-text-primary">Écrite</option>
                  </select>
                  <ChevronDown
                    size={14}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-text-tertiary pointer-events-none"
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none pl-4 pr-10 py-2.5 bg-white border border-border rounded-btn text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-blue/50 focus:border-blue transition-colors cursor-pointer"
                  >
                    <option value="EASY" className="bg-white text-text-primary">Facile</option>
                    <option value="MEDIUM" className="bg-white text-text-primary">Moyenne</option>
                    <option value="HARD" className="bg-white text-text-primary">Difficile</option>
                  </select>
                  <ChevronDown
                    size={14}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-text-tertiary pointer-events-none"
            ...
            <div>
              ...
              <textarea
                ...
                required
                rows={4}
                className="w-full pl-4 pr-4 py-2.5 bg-white border border-border rounded-btn text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-blue/50 focus:border-blue transition-colors resize-none"
                  ...
                  {options.map((option, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 pl-4 pr-4 py-2.5 bg-white border border-border rounded-btn text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-blue/50 focus:border-blue transition-colors"
                ...
                {isMCQ ? (
                  <div className="relative">
                    <select
                      ...
                      className="w-full appearance-none pl-4 pr-10 py-2.5 bg-white border border-border rounded-btn text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-blue/50 focus:border-blue transition-colors cursor-pointer"
                    >
                      <option value="" className="bg-white text-text-primary">Sélectionnez l'option correcte</option>
                      {options.map((opt, idx) => (
                        opt.trim() && (
                          <option key={idx} value={opt} className="bg-white text-text-primary">
                  ...
                  <input
                    ...
                    onChange={(e) => setCorrectAnswer(e.target.value)}
                    required
                    className="w-full pl-4 pr-4 py-2.5 bg-white border border-border rounded-btn text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-blue/50 focus:border-blue transition-colors"
