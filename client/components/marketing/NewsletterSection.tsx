  ...
  return (
    <section className="py-16 px-6 relative bg-[#F4F7F8]">
      ...
      <div className="relative z-10 max-w-xl mx-auto text-center">
        ...
        <h3 className="text-2xl md:text-3xl font-bold mb-3 text-[#102631]">
          {isFr ? 'Restez informé' : 'Stay in the loop'}
        </h3>
        <p className="text-[#586A73] mb-8 text-sm md:text-base">
        ...
        {status === "success" ? (
          <div className="flex items-center justify-center gap-3 px-6 py-4 rounded-xl bg-[#55A630]/10 border border-[#55A630]/30">
            <div className="w-8 h-8 rounded-full bg-[#55A630]/20 flex items-center justify-center">
              <Check className="w-5 h-5 text-[#55A630]" />
            </div>
            <p className="text-[#102631] font-medium">{message}</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative">
              <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#586A73]" />
              <input
                type="email"
                value={email}
